...
def create_indices(
    ...
    for i in range(len(episode_ends)):
        ...
        for _ in range(episode_repeats[i]):
...
class SequenceSampler:
    def __init__(self, 
        ...
        if episode_repeats is None:
            episode_repeats = np.ones(episode_ends.shape, dtype=np.int64)
        else:
            episode_repeats = np.asarray(episode_repeats)
            assert episode_repeats.shape == episode_ends.shape
            assert np.issubdtype(episode_repeats.dtype, np.integer)
            assert np.all(episode_repeats >= 1)
            episode_repeats = episode_repeats.astype(np.int64, copy=False)
