...
- H100 handoff was hardened after review: it begins fail-closed on clean `Lee` and verifies the implementation minimum `952aff2` is an ancestor (later documentation commits are allowed); it asserts two H100 names via `nvidia-smi` and two H100 JAX `device_kind` values; it creates/verifies the cache output root writable; it validates each v3 `meta/info.json` action/RGB/tactile/state schema plus encoder files; and cache/smoke/resume commands use `set -o pipefail` with preserved `tee` logs.
