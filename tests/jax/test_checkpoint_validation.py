...
def test_tactile_only_change_allows_identical_processor_and_stats_sidecars(
    tmp_path: Path,
) -> None:
    base_contract = replace(
        VT_CONTRACT,
...
def test_tactile_only_change_still_rejects_identical_base_config(tmp_path: Path) -> None:
    base_contract = replace(
        VT_CONTRACT,
        tactile_keys=(),
        tactile_num_tokens=0,
