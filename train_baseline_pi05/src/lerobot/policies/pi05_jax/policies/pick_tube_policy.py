...
class PickTubeInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        unknown = set(parsed) - set(self.image_keys)
        if unknown:
            raise ValueError(
                f"image keys must be a subset of {self.image_keys}, got extra {sorted(unknown)}"
            )
        ...
        inputs = {
            "state": data["state"],
            "image": {key: parsed.get(key, np.zeros_like(template)) for key in self.image_keys},
            "image_mask": {
                key: np.True_ if key in parsed else pad_mask for key in self.image_keys
            },
        }
