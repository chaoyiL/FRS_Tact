...
def _jitted_reverse_from_context(
    ...
    *,
    num_steps: int,
    solver: Literal["euler", "fireflow", "slerpflow"],
):
...
def sample_and_reverse(
    ...
    *,
    sample_steps: int,
    reverse_steps: int,
    solver: Literal["euler", "fireflow", "slerpflow"] = "euler",
    ...
    if solver not in ("euler", "fireflow", "slerpflow"):
        raise ValueError(f"solver must be 'euler' , 'fireflow' or 'slerpflow', got {solver!r}.")
