...
profile = root["meta"].attrs.get("state_action_profile", "dual-arm-20x20")
profile_dims = {
    "dual-arm-20x20": (20, 20, 2),
    "single-right-arm-7x10": (7, 10, 1),
}
if profile not in profile_dims:
    raise SystemExit(f"unsupported state_action_profile: {profile}")
state_dim, action_dim, arm_count = profile_dims[profile]
expected = {
    "camera1": (224, 224, 3),
    "camera2": (224, 224, 3),
    "observation_state": (state_dim,),
    "tactile_embedding": (tactile_embedding_dim,),
    "action": (action_dim,),
}
idle_shape = root["data"]["idle_arm_mask"].shape
if idle_shape[1:] != (arm_count,):
    raise SystemExit(
        f"idle_arm_mask: expected [N,{arm_count}], got {idle_shape}"
    )
