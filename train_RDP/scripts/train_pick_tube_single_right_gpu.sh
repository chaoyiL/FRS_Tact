...
OUTPUT_ROOT=${OUTPUT_ROOT:-${RDP_DIR}/data/outputs/insert_01_02_single_right}
RUN_ID=${RUN_ID:-single_right_pca30_$(date +%Y%m%d_%H%M%S)}
...
AT_PERIODIC_KEEP=${AT_PERIODIC_KEEP:-10}
LDP_PERIODIC_KEEP=${LDP_PERIODIC_KEEP:-10}
RESUME=${RESUME:-true}
...
usage() {
  cat <<'USAGE'
Usage: bash scripts/train_pick_tube_single_right_gpu.sh <at|ldp|all>

Examples:
  # Insert 01+02 single-right-arm training on physical GPU 0
  RUN_ID=single_right_pca30_v1 \
...
Common environment overrides:
  GPU_ID=0
  DATASET_PATH=/absolute/path/to/insert_01_02_pca30_single_right_rdp_zarr
  OUTPUT_ROOT=/absolute/path/to/outputs
  RUN_ID=single_right_pca30_v1
  AT_EPOCHS=20 LDP_EPOCHS=10
  AT_BATCH=64 LDP_BATCH=64 NUM_WORKERS=8
  TACTILE_DIM=30              # PCA output dimension from the existing four-sensor arm-wise artifact
  ...
  AT_PERIODIC_KEEP=10 LDP_PERIODIC_KEEP=10
...
for value_name in AT_CHECKPOINT_KEEP LDP_CHECKPOINT_KEEP AT_PERIODIC_KEEP LDP_PERIODIC_KEEP; do
  value=${!value_name}
  if [[ ! "${value}" =~ ^[0-9]+$ ]]; then
...
if (( ${#RUN_ID} > 64 )); then
  echo "RUN_ID must be at most 64 characters for W&B tags (${#RUN_ID}): ${RUN_ID}" >&2
  exit 2
fi
...
train_at() {
  local args=(
    "${PYTHON_BIN}" train.py
    --config-name=train_pick_tube_single_right_at_workspace
    ...
    "checkpoint.periodic.keep=${AT_PERIODIC_KEEP}"
...
train_ldp() {
  ...
  local args=(
    ...
    train.py
    --config-name=train_pick_tube_single_right_ldp_workspace
    "task.dataset_path=${DATASET_PATH}"
    "task.tactile_embedding_dim=${TACTILE_DIM}"
    "exp_name=${RUN_ID}"
