...
2026-09-06 起，四任务共用的 LDP 配置默认启用 RGB 光照增强（80% 概率，亮度/对比度 ±20%、饱和度 ±10%）。每个相机两帧使用一致参数，验证和部署不增强，触觉和 AT 不受影响。更新服务器代码后新启动的训练才会使用；建议使用新 RUN_ID，保留原权重作对照。详细配置见 `TRAIN_RDP_BASELINE.md`。
