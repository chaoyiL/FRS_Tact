"""
Baseline source: reactive_diffusion_policy-main/reactive_diffusion_policy/model/vae/model.py.
The original L1 reconstruction + sampled-posterior KL objective is unchanged.
Local engineering adaptations: portable torch.load, explicit device/freeze
helpers, and backward-compatible normalizer checkpoint persistence.
...
class VAE:
    def __init__(
        ...
        if load_dir is not None:
            try:
                state_dict = torch.load(load_dir, weights_only=False)
            except RuntimeError:
                state_dict = torch.load(load_dir, map_location=torch.device("cpu"), weights_only=False)
    ...
    def parameters(self):
        return iter(self.optim_params)

    def requires_grad_(self, requires_grad=True):
        for parameter in self.optim_params:
