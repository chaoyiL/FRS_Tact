"""Derived from the untouched original policy/latent_diffusion_unet_image_policy.py.

Original sampled-posterior diffusion MSE is unchanged. Local adaptations only
isolate shape metadata, freeze/move the tokenizer, preserve full decoded action
dimensions, and include tokenizer weights in checkpoints.
"""
...
class LatentDiffusionUnetImagePolicy(DiffusionUnetImagePolicy):
    def __init__(self,
        ...
        original_action_dim = shape_meta['action']['shape'][0]
        if at.use_vq:
        ...
        self.at = at
        self.at.requires_grad_(False).eval()
    ...
    def get_extra_state(self):
        # VAE intentionally retains the upstream plain-object interface, so
        # nn.Module would otherwise omit its frozen weights from checkpoints.
        return {'at': self.at.state_dict()}

    def set_extra_state(self, state):
        self.at.load_state_dict(state['at'])
        self.at.requires_grad_(False).eval()
    ...
    def predict_action(self,
        ...
        if return_latent_action:
            ...
            # unnormalize action
            naction_pred = nsample[..., :self.original_action_dim]
    ...
    def compute_loss(self, batch):
        ...
        # AT is a fixed tokenizer; posterior targets remain sampled as upstream.
        with torch.no_grad():
