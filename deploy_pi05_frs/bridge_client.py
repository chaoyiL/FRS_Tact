...
def _is_tunnel_host(host: str | None) -> bool:
    if host is None:
        return False
    normalized = host.rstrip(".").lower()
    return any(normalized == suffix or normalized.endswith(f".{suffix}") for suffix in _TUNNEL_HOST_SUFFIXES)
...
class RobotBridgeClient:
    ...
    def _connect(self) -> ClientConnection:
        ...
        while True:
            try:
                ...
                print(f"[bridge] Connection failed: {error!r}; retrying in {self.retry_interval_s:.1f}s")
                time.sleep(self.retry_interval_s)
            except InvalidStatus as error:
                raise RuntimeError(
                    f"Robot bridge rejected the handshake with HTTP {error.response.status_code}; check token"
                ) from error
    ...
    def receive_action_ack(self, obs_seq: int, timeout: float) -> None:
        ...
        if message.get("type") != "action_ack":
            raise RuntimeError(f"Expected action acknowledgement, received: {message.get('type')}")
        ...
        if (
            ...
            raise RuntimeError(
                f"Expected action acknowledgement for observation {obs_seq}, received: {acknowledged_obs_seq}"
            )
