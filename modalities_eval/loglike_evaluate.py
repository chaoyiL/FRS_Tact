...
_RUN_SCAN_CACHE: dict[tuple[Any, ...], Any] = {}
...
def _batch_dot(a: jax.Array, b: jax.Array) -> jax.Array:
    axes = tuple(range(1, a.ndim))
    return jnp.sum(a * b, axis=axes, keepdims=True)


def _batch_norm(a: jax.Array) -> jax.Array:
    axes = tuple(range(1, a.ndim))
    return jnp.sqrt(jnp.sum(jnp.square(a), axis=axes, keepdims=True) + 1e-8)


def _slerp_unit_directions(u: jax.Array, v: jax.Array, *, alpha: float = 0.5) -> jax.Array:
    """Spherical linear interpolation of unit directions (matches utils.integration)."""

    dot = jnp.clip(_batch_dot(u, v), -1.0 + 1e-7, 1.0 - 1e-7)
    ...
    slerp_res = (jnp.sin((1.0 - alpha) * omega) / sin_omega_safe) * u + (
        jnp.sin(alpha * omega) / sin_omega_safe
    ) * v
    d_next = jnp.where(omega < 1e-6, lerp_res, slerp_res)
    return d_next / _batch_norm(d_next)


def slerpflow_step_velocity(
    z_t: jax.Array,
    v_curr: jax.Array,
    dt: jax.Array,
    t_next: jax.Array,
    ...
    """Return (v_slerp, v_next) for one SlerpFlow step with fixed v_curr."""

    z_euler = z_t + dt * v_curr
    v_next = velocity_fn(z_euler, t_next)
    v_bar = (v_curr + v_next) / 2.0

    rho_t = _batch_norm(z_t)
    d_t = z_t / rho_t
    rho_next = rho_t + dt * _batch_dot(d_t, v_bar)

    z_bar = z_t + dt * v_bar
    d_euler = z_bar / _batch_norm(z_bar)
...
def _get_likelihood_scan(
    ...
    *,
    ...
    def run_scan(context: VelocityContext, x: jax.Array, step_indices: jax.Array):
        ...
        if ode_solver == ODE_SOLVER_EULER:
        ...
        elif ode_solver == ODE_SOLVER_FIREFLOW:
            ...
            x, r_tot, _ = _run_slerpflow_likelihood_scan(
                x=x,
                r_tot=r_tot,
                t=t,
                step_indices=step_indices,
