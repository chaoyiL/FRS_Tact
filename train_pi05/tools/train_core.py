...
def _load_weights_and_validate(loader: _weight_loaders.WeightLoader, params_shape: at.Params) -> at.Params:
    ...
    try:
        ...
        print("none of dim needs to be changed")
        pass  # ← 新增代码结束

    # Fill missing keys (including nested keys) with ShapeDtypeStruct so they get randomly initialized.
    def _fill_missing(expected_tree, got_tree, path=""):
        if not isinstance(expected_tree, dict) or not isinstance(got_tree, dict):
            return
        for key, expected_value in expected_tree.items():
            key_path = f"{path}/{key}" if path else str(key)
            if key not in got_tree:
                logging.info(f"[COMPAT] Key '{key_path}' not in checkpoint, will be randomly initialized.")
                got_tree[key] = expected_value
                continue
            _fill_missing(expected_value, got_tree[key], key_path)

    _fill_missing(params_shape, loaded_params)
...
def init_train_state(
    ...
    def init(rng: at.KeyArrayLike, partial_params: at.Params | None = None) -> training_utils.TrainState:
        ...
        # Always split/merge to ensure consistent graphdef between eval_shape and jit.
