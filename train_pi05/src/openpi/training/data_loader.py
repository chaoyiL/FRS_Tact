...
class ActionKeyDataset(Dataset):
    """Expose each source action column under the canonical ``actions`` key."""

    def __init__(self, dataset: Dataset, action_key: str):
        self._dataset = dataset
        self._action_key = action_key

    def __getitem__(self, index: SupportsIndex) -> dict:
        sample = dict(self._dataset[index])
        sample["actions"] = sample[self._action_key]
        return sample
...
def create_torch_dataset(
    ...
    *,
    ...
    for source in sources:
        ...
        # the VB input transforms convert it directly to the model prompt.
    ...
    if data_config.repo_id != "fake" and not skip_norm_stats:
        if data_config.norm_stats is None:
            raise ValueError(
                ...
                "Make sure to run `tools/compute_norm_stats.py <config-name>`."
            )
