...
class DataConfig:
    ...
    # Ordered local/Hub datasets. When non-empty they are concatenated in this
    # order and replace the legacy single repo_id source.
...
class TrainConfig:
    ...
    # Optional exact output directory used by the standalone YAML launcher.
    ...
    def checkpoint_dir(self) -> pathlib.Path:
        ...
        if self.checkpoint_dir_override is not None:
            return pathlib.Path(self.checkpoint_dir_override).expanduser().resolve()
...
# ============================================================================
# Data collection config (replaces 00_get_data.py runtime params + VB_task_config.yaml recorder section)
# ============================================================================
@dataclasses.dataclass
class DataCollectConfig:
    """Unified configuration for raw data collection.

    This replaces the 00_get_data.py CLI/YAML split as the single source of
    truth for camera recording, Quest pose receiving, and Quest control
    signaling parameters.
    """
    # ---- Task identity ----
    task_name: str = "raw_0118_data"
    task_type: str = "bimanual"       # "single" or "bimanual"
    single_hand_side: str = "left"    # only used when task_type == "single"

    # ---- Output layout ----
    output_dir: str = "./data"
    demos_dir_name: str = "demos"
    trajectory_dir_name: str = "all_trajectory"

    # ---- Camera capture (00_get_data.py) ----
    camera_format: str = "MJPG"
    camera_width: int = 3840
    camera_height: int = 800
    camera_auto_exposure: int = 3      # 1=auto, 3=manual
    camera_exposure: int | None = 400
    camera_auto_white_balance: int = 0 # 0=manual, 1=auto
    camera_wb_temperature: int | None = 4600
    camera_brightness: int = 0
    camera_gain: int = 100
    camera_gamma: int = 100
    left_hand_camera_path: str = "/dev/video0"
    right_hand_camera_path: str = "/dev/video2"
    camera_queue_size: int = 64
    camera_fps_interval: float = 1.0

    # ---- Quest pose receiving ----
    enable_pose: bool = True
    pose_host: str = "localhost"
    pose_port: int = 7777
    pose_frames_per_file: int = 5000
    pose_socket_timeout: float = 1.0
    pose_recv_buffer_size: int = 1024
    pose_buffer_max_size: int = 10 * 1024 * 1024
    pose_fps_window_size: int = 30
    pose_display_interval: float = 0.5

    # ---- Quest control channel ----
    control_host: str = "localhost"
    control_port: int = 50010
    control_connect_timeout: float = 1.0
    control_fps_send_interval: float = 0.5

    # ---- Runtime/UI tuning ----
    status_min_interval: float = 0.2
    idle_sleep_seconds: float = 0.001
    camera_worker_join_timeout: float = 1.0
    pose_join_timeout: float = 2.0
...
DATASET_TRAIN_NAME: str = "0118_data_smooth"
...
# Edit this instance to configure raw data collection.
DATA_COLLECT_CONFIG = DataCollectConfig(
    task_name=DATASET_RAW_TASK_NAME,
    task_type="bimanual",
    single_hand_side="left",
    output_dir="./data",
    demos_dir_name="demos",
    trajectory_dir_name="all_trajectory",
    # Camera capture
    camera_format="MJPG",
    camera_width=3840,
    camera_height=800,
    ...
    camera_exposure=400,
    camera_auto_white_balance=0,
    camera_wb_temperature=4600,
    camera_brightness=0,
    camera_gain=100,
    camera_gamma=100,
    left_hand_camera_path="/dev/video0",
    right_hand_camera_path="/dev/video2",
    camera_queue_size=64,
    camera_fps_interval=1.0,
    # Quest pose receiving
    enable_pose=True,
    pose_host="localhost",
    pose_port=7777,
    pose_frames_per_file=5000,
    pose_socket_timeout=1.0,
    pose_recv_buffer_size=1024,
    pose_buffer_max_size=10 * 1024 * 1024,
    pose_fps_window_size=30,
    pose_display_interval=0.5,
    # Quest control channel
    control_host="localhost",
    control_port=50010,
    control_connect_timeout=1.0,
    control_fps_send_interval=0.5,
    # Runtime/UI
    status_min_interval=0.2,
    idle_sleep_seconds=0.001,
    camera_worker_join_timeout=1.0,
    pose_join_timeout=2.0,
)
...
_CONFIGS = [
    TrainConfig(
        ...
        exp_name=data_name,
        # Load pretrained PaliGemma and action-expert weights.
