...
class DataHelpersTest(unittest.TestCase):
    def test_mp_worker_resolves_target_encoder_runtime(self):
        from train_pi05_frs.utils.mp_batches import _worker_loop

        task_q = mock.Mock()
        task_q.get.return_value = None
        result_q = mock.Mock()
        fake_factory_result = SimpleNamespace(dataset=object())
        with mock.patch(
            "train_encoder.utils.image_dataset.create_image_dataset",
            return_value=fake_factory_result,
        ) as create_image_dataset:
            _worker_loop(
                task_q,
                result_q,
                {
        ...
        create_image_dataset.assert_called_once_with(
            "org/demo",
            image_size=8,
            cache_size=256,
        )
