...
def _quadrant_masks(
    gates: np.ndarray, low: float, high: float
) -> dict[str, np.ndarray]:
    is_low = gates <= low
    is_high = gates >= high
    return {
...
def bimanual_quadrant_metrics(
    *,
    ...
    Inputs have shape ``[N, 2]`` in fixed left/right order. Boundaries are
    inclusive, mid-region samples are omitted, and empty groups retain NaNs.
    """
    ...
    arrays = tuple(
        _as_bimanual_array(value)
        for value in (mse_gt, mse_vla, mse_vla_gt, gate_weights)
    )
    ...
    for quadrant, mask in _quadrant_masks(gates, low, high).items():
        ...
        for wrist_index, wrist in enumerate(BIMANUAL_WRISTS):
            if not np.any(mask):
                group[wrist] = {
                    metric: float("nan") for metric in _WRIST_METRICS
                }
                continue
            ...
            group[wrist] = {
                ...
                "rank_satisfied_frac": float(
                    np.mean(
                        gt[mask, wrist_index] + margin <= vla[mask, wrist_index]
                    )
                ),
            }
    ...
    for quadrant in BIMANUAL_QUADRANTS:
        ...
        for wrist in BIMANUAL_WRISTS:
            ...
            for metric in _WRIST_METRICS:
                flattened[f"{prefix}_{quadrant}_{metric}_{wrist}"] = float(
                    wrist_metrics[metric]
                )
    return flattened
...
def bimanual_gate_region_counts(
    ...
    regions = np.full(gates.shape, 1, dtype=np.intp)
    regions[gates <= low] = 0
