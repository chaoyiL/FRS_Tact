...
usage() {
    cat <<'EOF'
Usage: bash deploy_pi05/scripts/start_remote_client.sh --config PATH [--mode pi05|frs] [--check] [--max-iterations N]
...
case "${MODE}" in
    pi05)
        ENTRYPOINT="deploy_pi05.pi05_client"
        MODE_PYTHON="${PI05_PYTHON:-}"
        ;;
    frs)
        ENTRYPOINT="deploy_pi05.remote_client"
...
export PYTHONPATH="${DEPLOY_ROOT}/src:${DEPLOY_ROOT}:${ROOT}${PYTHONPATH:+:${PYTHONPATH}}"
CONFIG_ABSOLUTE="$(cd "$(dirname "${CONFIG}")" && pwd)/$(basename "${CONFIG}")"
PYTHON_ARGS=(-m "${ENTRYPOINT}" --config "${CONFIG_ABSOLUTE}")
