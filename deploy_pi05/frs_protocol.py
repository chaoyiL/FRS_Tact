...
type FRSServerMessage = FRSChunkStart | FRSSteerRequest | FRSSteerAck | FRSChunkEnd
...
def _exact_keys(message: Mapping[str, Any], expected: frozenset[str]) -> None:
    ...
    if extras:
        raise FRSProtocolError(f"unexpected FRS message fields: {sorted(map(repr, extras))}")
...
def _parse_chunk_start(message: Mapping[str, Any]) -> FRSChunkStart:
    ...
    if execution_mode == "rtc":
        if (
            ...
            raise FRSProtocolError("RTC action_timestamps must be a finite floating ndarray with shape [H]")
        if not isinstance(nominal_chunk_end, (float, np.floating)) or not math.isfinite(
            float(nominal_chunk_end)
        ):
        ...
        if action_timestamps is not None or nominal_chunk_end is not None:
            raise FRSProtocolError("block action_timestamps and nominal_chunk_end must both be null")
...
def _parse_steer_ack(message: Mapping[str, Any]) -> FRSSteerAck:
    ...
    if (status == "scheduled") != (scheduled_timestamp is not None):
        raise FRSProtocolError("scheduled_timestamp must be finite exactly when status is 'scheduled'")
    return FRSSteerAck(
        chunk_id=_nonnegative_id(message, "chunk_id"),
        request_id=_nonnegative_id(message, "request_id"),
        action_index=_nonnegative_id(message, "action_index"),
