...
from utils import pi05_source_model
...
def test_pi05_jit_cache_key_separates_image_contracts() -> None:
    class FakeModel:
        action_dim = 20
        action_horizon = 50
        max_token_len = 200
    ...
    two_images = FakeModel(("left_wrist_0_rgb", "right_wrist_0_rgb"))
    three_images = FakeModel(("base_0_rgb", "left_wrist_0_rgb", "right_wrist_0_rgb"))

    two_key = pi05_source_model._cache_key(two_images)
    three_key = pi05_source_model._cache_key(three_images)
    assert two_images.image_keys in two_key
    assert three_images.image_keys in three_key
