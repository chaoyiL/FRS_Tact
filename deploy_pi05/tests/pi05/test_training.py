...
from lerobot.policies.pi05_jax import image_tools as local_image_tools
...
def test_transforms_use_the_vendored_jax_image_tools() -> None:
    assert transforms.image_tools is local_image_tools
    resized = transforms.ResizeImages(height=8, width=8)(
        {"image": {"left_wrist_0_rgb": np.zeros((5, 7, 3), dtype=np.uint8)}}
    )
    image = resized["image"]["left_wrist_0_rgb"]
    assert image.shape == (8, 8, 3)
    assert image.dtype == np.uint8


def test_resize_images_directly_stretches_full_visual_frame_without_padding() -> None:
    frame = np.full((5, 7, 3), 255, dtype=np.uint8)

    resized = transforms.ResizeImages(height=8, width=8)(
        {"image": {"left_wrist_0_rgb": frame}}
    )["image"]["left_wrist_0_rgb"]

    np.testing.assert_array_equal(np.asarray(resized), np.full((8, 8, 3), 255, dtype=np.uint8))
...
def test_pick_tube_inputs_emits_exact_configured_two_image_contract() -> None:
    ...
    out = PickTubeInputs(model_type=ModelType.PI05, image_keys=image_keys)(
        {
    ...
    assert tuple(out["image"]) == image_keys
    assert tuple(out["image_mask"]) == image_keys
...
def test_pi0_config_inputs_spec_respects_configured_image_keys() -> None:
    image_keys = ("left_wrist_0_rgb", "right_wrist_0_rgb")
    config = Pi0Config(pi05=True, image_keys=image_keys)

    observation, _ = config.inputs_spec()

    assert tuple(observation.images) == image_keys
    assert tuple(observation.image_masks) == image_keys
