...
class FakeBridge:
    ...
    def receive_action_ack(self, obs_seq, timeout):
        raise AssertionError("plain ManiSkill-compatible deployment must not wait for action_ack")
...
def test_legacy_loop_sends_action_then_waits_for_next_observation_without_ack():
...
def test_legacy_loop_stops_at_max_iterations():
    ...
    assert [event[0] for event in bridge.events] == ["receive", "send_action"]
...
def _patch_run_dependencies(monkeypatch, bridge, policy, saver):
    ...
    monkeypatch.setattr(
        pi05_client,
        "_jax_runtime",
        lambda: ("cpu", (SimpleNamespace(platform="cpu", id=0, device_kind="CPU"),)),
        raising=False,
    )
    monkeypatch.setattr(
        pi05_client,
...
def test_main_configures_concise_logging_before_run(monkeypatch, tmp_path):
    events = []
    monkeypatch.setattr(
        pi05_client,
    ...
    monkeypatch.setattr(
        pi05_client,
        "run",
        lambda config, max_iterations: events.append(("run", config, max_iterations)),
    )

    assert pi05_client.main(["--config", str(tmp_path / "deploy.yaml")]) == 0
    assert events == ["logging", ("run", tmp_path / "deploy.yaml", None)]


def test_run_prints_summary_before_loading_policy(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        pi05_client,
        "print_startup_summary",
        lambda *args, **kwargs: startup_events.append("summary"),
        raising=False,
    )
    monkeypatch.setattr(
        pi05_client,
        "_make_policy",
        lambda config: startup_events.append("policy") or policy,
    )

    pi05_client.run(tmp_path / "deploy.yaml")

    assert startup_events == ["summary", "policy"]
    output = capsys.readouterr().out
    assert "[startup] Loading pi0.5 model" in output
    assert "[startup] pi0.5 model loaded" in output
...
def test_run_attempts_stop_and_close_when_inference_raises(monkeypatch, tmp_path):
    bridge = FakeBridge(observations=[(1, _observation()), (2, _observation())])
    policy = FakePolicy(fail_on_prediction=2)
    saver = FakeSaver()
    _patch_run_dependencies(monkeypatch, bridge, policy, saver)
