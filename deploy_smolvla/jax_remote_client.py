"""Run JAX SmolVLA for FRS deployment against ``vb3_robot_server``."""
...
DEFAULT_CONFIG = Path(__file__).resolve().parent / "configs" / "deploy_frs.yaml"
...
def _direct_decoder_config(config: Mapping[str, Any]) -> Mapping[str, Any] | None:
    backend = str(config.get("backend", "jax_smolvla_frs"))
    if backend in {"jax_smolvla", "jax_smolvla_frs"}:
        return None
